...
#if !defined(BS_NODISGUARD)
# if __cplusplus < 201700L
#   if _MSC_VER >= 1700
#       define BS_NODISGUARD // should be _Check_return_ but MS wont accept my macro alias
#   else
#       define BS_NODISGUARD // __attribute__((warn_unused_result))
#   endif
# else
#   define BS_NODISGUARD [[nodisguard]]
# endif
#endif
...
using bs_invoke_result_t =  typename invoke_result<Ts...>::type;


template< class T >
struct bs_is_void : std::is_same<void, typename std::remove_cv<T>::type> {};

template<typename T>
constexpr bool bs_is_void_v = bs_is_void<T>::value;


#else

template<typename ... Ts>
using bs_invoke_result_t = std::invoke_result_t<Ts...>;

template<typename T>
constexpr bool bs_is_void_v = std::is_void_v<T>;
...
namespace BS
{
	 ...
	 */
	using concurrency_t = bs_invoke_result_t<decltype(std::thread::hardware_concurrency)>;
	...
	template <typename T >
	struct vector_return_type
	{
		BS_NODISGUARD std::vector<T> get()
		{
	...
	template<typename T >
	class multi_future : public std::conditional_t< std::is_same<T, void>::value, void_return_type, vector_return_type<T> >
	{
		using parent_type = std::conditional_t< std::is_same<T, void>::value, void_return_type, vector_return_type<T> >;
	public:
		/**
		...
		{
			parent_type::futures.resize(num_futures_);
		}
		 ...
		 */
		  ...
		  * @return The future.
		  */
		BS_NODISGUARD std::future<T>& operator[](const size_t i)
		{
			return parent_type::futures[i];
		}
		 ...
		 */
		BS_NODISGUARD void push_back(std::future<T> future)
		{
			parent_type::futures.push_back(std::move(future));
		}
		 ...
		 */
		BS_NODISGUARD size_t size() const
		{
			return parent_type::futures.size();
		}
		...
		void wait() const
		{
			for (size_t i = 0; i < parent_type::futures.size(); ++i)
				parent_type::futures[i].wait();
		}
	...
	public:
		 ...
		 */
		BS_NODISGUARD T start(const size_t i) const
		{
		 ...
		 */
		BS_NODISGUARD T end(const size_t i) const
		{
		 ...
		 */
		BS_NODISGUARD size_t get_num_blocks() const
		{
			return num_blocks;
		}
		 ...
		 */
		BS_NODISGUARD size_t get_total_size() const
		{
			return total_size;
		}
	...
	public:
		 ...
		 */
		BS_NODISGUARD size_t get_tasks_queued() const
		{
		 ...
		 */
		BS_NODISGUARD size_t get_tasks_running() const
		{
			std::unique_lock<std::mutex> tasks_lock(tasks_mutex);
		 ...
		 */
		BS_NODISGUARD size_t get_tasks_total() const
		{
			return tasks_total;
		}
		 ...
		 */
		BS_NODISGUARD concurrency_t get_thread_count() const
		{
			return thread_count;
		}
		 ...
		 */
		BS_NODISGUARD bool is_paused() const
		{
			return paused;
		}
		 ...
		 */
		template <typename F, typename T1, typename T2, typename T = std::common_type_t<T1, T2>, typename R = bs_invoke_result_t<std::decay_t<F>, T, T>>
		BS_NODISGUARD multi_future<R> parallelize_loop(const T1 first_index, const T2 index_after_last, F&& loop, const size_t num_blocks = 0)
		{
			blocks<T1,T2> blks(first_index, index_after_last, num_blocks ? num_blocks : thread_count);
		 ...
		 * @return A multi_future object that can be used to wait for all the blocks to finish. If the loop function returns a value, the multi_future object can also be used to obtain the values returned by each block.
		 */
		template <typename F, typename T, typename R = bs_invoke_result_t<std::decay_t<F>, T, T>>
		BS_NODISGUARD multi_future<R> parallelize_loop(const T index_after_last, F&& loop, const size_t num_blocks = 0)
		{
		 ...
		 */
		template <typename F, typename... A, typename R = bs_invoke_result_t<std::decay_t<F>, std::decay_t<A>...>,
			std::enable_if_t<bs_is_void_v<R>, bool> = true >
		BS_NODISGUARD std::future<R> submit(F&& task, A&&... args)
		{
	...
	private:
		 ...
		 */
		BS_NODISGUARD concurrency_t determine_thread_count(const concurrency_t thread_count_)
		{
			if (thread_count_ > 0)
	...
	public:
		...
		// */
		static constexpr std::ostream& (&endl)(std::ostream&) = static_cast<std::ostream & (&)(std::ostream&)>(std::endl);

		///**
		// * @brief A stream manipulator to pass to a synced_stream (an explicit cast of std::flush). Used to flush the stream.
		// */
		static constexpr std::ostream& (&flush)(std::ostream&) = static_cast<std::ostream & (&)(std::ostream&)>(std::flush);
